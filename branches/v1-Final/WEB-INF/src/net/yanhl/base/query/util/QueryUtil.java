...
public class QueryUtil {
	...
	private synchronized Map<String, String[]> getParameterMap(ListQuery listQuery) {
		...
		for (Iterator<String> iter = keys.iterator(); iter.hasNext();) {
			...
			String paramValue = StringUtil.getValue(request, paramKey);
			// 获得逻辑和比较运算符
	...
	public synchronized void setCommonQuery(Session session) {
		...
		if(!listQuery.getCustomFilter().isEmpty()) {
			Iterator<String[]> customIter = listQuery.getCustomFilter().iterator();
			while (customIter.hasNext()) {
				String[] customFilter = (String[]) customIter.next();
				String result = customFilter[0] + " " + customFilter[1] + customFilter[2];
				if(needWhere()) {
	...
	private String getTables(Map<String, String[]> queryTerm, Map<String, String> tableMap) {
		...
		while (iterator.hasNext()) {
			String[] customFilter = (String[]) iterator.next();
			//逻辑比较符号左边
			if(customFilter[0].indexOf(".") != -1) {// 如果是以别名+属性名
				String queryAffix = customFilter[0].split("\\.")[0];
				// 增加查询的表
				if (!queryAffix.equals("o")) {
			...
			//逻辑比较符号右边
			if(customFilter[2].indexOf(".") != -1) {// 如果是以别名+属性名
				String queryAffix = customFilter[2].split("\\.")[0];
				// 增加查询的表
				if (!queryAffix.equals("o")) {
					if(tables.indexOf(tableMap.get(queryAffix)) == -1) {
						tables += "," + tableMap.get(queryAffix) + " " + queryAffix;
